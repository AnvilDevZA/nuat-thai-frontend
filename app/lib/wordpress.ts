...
export async function getAllProducts(filterParams?: {
  product_tag?: string;
  product_cat?: string;
  search?: string;
}): Promise<Page[]> {
  ...
  if (filterParams?.search) {
    ...
    if (filterParams?.product_tag) {
      query.tags = filterParams.product_tag;
    }
    if (filterParams?.product_cat) {
      query.categories = filterParams.product_cat;
    }
  } else {
    ...
    if (filterParams?.product_cat) {
     ...
     const url = getUrl("/wp-json/wp/v2/product", query);
